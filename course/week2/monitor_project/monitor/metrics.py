...
def get_hist_score(tr_probs, te_probs, bins=10):
  ...
  # Compute histograms with density=True to normalize the heights
  ...
  # Loop through bins to compute the intersection area
...
def get_vocab_outlier(tr_vocab, te_vocab):
  ...
  # score: float (between 0 and 1)
  # Count the number of words in the test vocabulary that are also in the training vocabulary
  num_seen = sum(1 for word in te_vocab if word in tr_vocab)
  
  # Calculate the total number of unique words in the test vocabulary
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    # Convert PyTorch tensors to NumPy arrays directly if they are already on the CPU
    tr_probs_np = tr_probs.numpy()
    ...
    tr_probs_cal_np = iso_regression.fit_transform(tr_probs_np, tr_labels_np)
    
    # Calibrate the test probabilities
    te_probs_cal_np = iso_regression.transform(te_probs_np)
    
    # Convert the calibrated probabilities back to PyTorch tensors
    tr_probs_cal = torch.tensor(tr_probs_cal_np, dtype=tr_probs.dtype, device=tr_probs.device)
    te_probs_cal = torch.tensor(te_probs_cal_np, dtype=te_probs.dtype, device=te_probs.device)
